...
static arma::vec get_ideal_utilites(Qrels &qrels, AMTSimulation &utility_scores,
    ...
    for (int i = 0; i < rank; i++) {
        ...
        if(i == 0){
            ...
            int max_doc = -1;
            for(int docIndex=0; docIndex < utility_scores.allDocs.size(); docIndex++){
...
static arma::vec get_doc_utilites(Qrels &qrels, vector<Document> run,
                                AMTSimulation& utility_scores, int rank, string fType){
    ...
    for (int i = 0; i < min(rank, int(run.size())); i++) {
        find_doc = utility_scores.doc_index_map.find(run.at(i));
        if(find_doc != utility_scores.doc_index_map.end())
        ...
        if(docIndex == -1)
            utility(i) = 0;
        else if(rankList_docIndex.size() == 0){
            utility(i) = utility_scores.get_UtilityScore(docIndex);
            rankList_docIndex.push_back(docIndex);
        } else{
            arma::vec this_doc_utilities = arma::zeros(rankList_docIndex.size());

            for(int seenDoc=0; seenDoc < rankList_docIndex.size(); seenDoc++)
                this_doc_utilities(seenDoc) = utility_scores.get_UtilityScore(docIndex, rankList_docIndex[seenDoc]);
 ...
 static arma::vec compute_pref_score(arma::vec doc_utility, arma::vec ideal_utility,
    ...
    //ideal_utility.print("idea");
    //doc_utility.print("doc_utility");
    //cout << pType << endl;
...
static void simulationTesting(AMTSimulation& utility_scores, Qrels& qrels){
    cout << endl;
    cout << endl;
    set<double> classes;
...
static map<string, arma::vec> pref_measure(vector<Document>& run, Qrels& qrels, int rank, AMTSimulation &utility_scores){
...
static void pref_measure_printIdeal(Qrels& qrels, int rank, AMTSimulation &utility_scores, int e , int m){
    //simulationTesting(utility_scores, qrels);
